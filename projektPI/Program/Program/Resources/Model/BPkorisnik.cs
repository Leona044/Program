...
namespace Program
{
    public static class BPkorisnik
    {
        ...
        public static List<Korisnik> DohvatiPrijava(string email, string lozinka)
        {
            List<Korisnik> listaKorisnika = new List<Korisnik>();
            BP.otvoriKonekciju();

            SQLiteCommand command = BP.konekcija.CreateCommand();
            command.CommandText = String.Format("SELECT * FROM Korisnik WHERE email = '" + email + "'and lozinka = '" + lozinka + "'");
            SQLiteDataReader reader = command.ExecuteReader();
            while (reader.Read())
            {
                Korisnik k = new Korisnik();
